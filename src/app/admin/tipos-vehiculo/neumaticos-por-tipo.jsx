...
export default function NeumaticosPorTipo({ tipoId }) {
  const [neumaticos, setNeumaticos] = useState([]);
  const [loading, setLoading] = useState(true);
  const router = useRouter();

  useEffect(() => {
    if (tipoId) fetchNeumaticos();
  ...
  if (neumaticos.length === 0) return <div style={{ color: "#64748b" }}>No hay neumáticos asociados a este tipo de vehículo.</div>;
